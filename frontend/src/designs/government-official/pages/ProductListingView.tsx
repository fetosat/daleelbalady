...
export default function ProductListingView({ data, type }: ProductListingViewProps) {
  ...
  const hasDiscount = data.comparePrice && data.price && data.comparePrice > data.price
  const discountPercent = hasDiscount && data.price
            ...
            <div className="space-y-2">
              <div className="flex items-baseline gap-3">
                <span className="text-4xl font-bold text-primary">
                  {data.currency || 'EGP'} {(data.price || 0).toFixed(2)}
                </span>
                {hasDiscount && (
