...
import { ListingType, ListingFormData } from '@/types/dashboard';
import { X, Store, Package, ShoppingBag, Upload, ArrowLeft, ArrowRight, Check, Trash2 } from 'lucide-react';
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className=" bg-white dark:bg-stone-800 rounded-2xl shadow-2xl z-50 overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-stone-200">
              <div>
                ...
                <p className="text-sm text-stone-600 mt-1">
                  Step {['type', 'details', 'media', 'review'].indexOf(step) + 1} of 4
                </p>
              </div>
              <button
                onClick={handleClose}
            ...
            <div className="h-1 bg-stone-200">
              <motion.div
                ...
                initial={{ width: '0%' }}
                animate={{
                  width: `${((['type', 'details', 'media', 'review'].indexOf(step) + 1) / 4) * 100}%`
                }}
            ...
            <div className="p-6 max-h-[60vh] overflow-y-auto">
              <AnimatePresence mode="wait">
                {step === 'type' && (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      {[
                        { type: 'SHOP' as ListingType, icon: Store, label: 'Shop', description: 'Physical or online store' },
                        { type: 'SERVICE' as ListingType, icon: ShoppingBag, label: 'Service', description: 'Bookable service' },
                        { type: 'PRODUCT' as ListingType, icon: Package, label: 'Product', description: 'Sellable item' },
                      ].map((option) => {
                        ...
                        return (
                          ...
                          >
                            <div className={cn(
                              'p-4 rounded-xl',
                              isSelected ? 'bg-blue-100' : 'bg-stone-100'
                            )}>
                              <Icon className={cn(
                                'h-8 w-8',
                                isSelected ? 'text-blue-600' : 'text-stone-600'
                              )} />
                            </div>
                ...
                {step === 'review' && (
                  ...
                  exit={{ opacity: 0, x: -20 }}
                  >
                    <h3 className="text-lg font-semibold text-stone-900 mb-4">Review & Create</h3>
            ...
            <div className="flex items-center justify-between p-6 border-t border-stone-200 bg-stone-50">
              <button
                onClick={() => {
                  const steps: Step[] = ['type', 'details', 'media', 'review'];
